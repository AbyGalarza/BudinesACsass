...
const renderizarElemento = ()=>{
    ...
    if(carrito.length > 0){
        for (let index = 0; index < carrito.length; index++) {
            ...
            sumaTotal = sumaTotal + (Number(carrito[index].precio.substring(1)) * carrito[index].cantidad);  //se usa el metodo substring para remover el '$' del precio y la funcion Number() para convertir el string a numero
            listadoCarrito.append(elemento);
            total.innerText = `Total a pagar: $${sumaTotal}`;  //se agrega un '$' al total al mostrarlo en el DOM
        }
    }
}
